...
#' @export
split_training_and_test_sets <- function(x,
                                         split_var,
                                         referece_label,
                                         query_label,
                                         reduction = "harmony",
  ...
  if (is(x, "Seurat")) {
    ...
    cells <- colnames(x)
    reference_cells <- cells[x@meta.data[[split_var]] == referece_label]
    query_cells <- cells[x@meta.data[[split_var]] == query_label]


    # Batch-corrected principal components are used as explanatory variables
    cell_embeddings <- Embeddings(x, reduction = "harmony")
